...
const SettingsManager = {
    ...
    loadSettings()
    {
        if (!document.getElementById('settings_pageWidth')) {
            return;
        }
...
function applyMiniPrintoutConfig(settings = {})
{
    ...
    const buildFontStyle = (family, size, bold, italics, underline, strike) => {
        const styles = [];
        if (toBool(bold, false)) styles.push('Bold');
        if (toBool(italics, false)) styles.push('Italic');
        if (toBool(underline, false)) styles.push('Underline');
        if (toBool(strike, false)) styles.push('Strikeout');
        return `${family}, ${size}pt` + (styles.length ? `, style=${styles.join(',')}` : '');
    };

    LAYOUT.PAGE_WIDTH = toNumber(settings.pageWidth, LAYOUT.PAGE_WIDTH);
    LAYOUT.MARGIN_LEFT = toNumber(settings.marginsLeft, LAYOUT.MARGIN_LEFT);
    LAYOUT.MARGIN_RIGHT = toNumber(settings.marginsRight, LAYOUT.MARGIN_RIGHT);
    LAYOUT.MARGIN_TOP = toNumber(settings.marginsTop, LAYOUT.MARGIN_TOP);
    LAYOUT.MARGIN_BOTTOM = toNumber(settings.marginsBottom, LAYOUT.MARGIN_BOTTOM);
    LAYOUT.LABEL_HEIGHT = toNumber(settings.labelHeight, LAYOUT.LABEL_HEIGHT);
    LAYOUT.INPUT_HEIGHT = toNumber(settings.outputHeight, LAYOUT.INPUT_HEIGHT);
    LAYOUT.VERTICAL_SPACING = toNumber(settings.verticalSpacing, LAYOUT.VERTICAL_SPACING);

    ConversionSettings.alignToGrid = toBool(settings.alignToGrid, ConversionSettings.alignToGrid);
    ConversionSettings.snapGridSize = toNumber(settings.snapGridSize, ConversionSettings.snapGridSize);

    LAYOUT.FONT_REPORTHEADER = buildFontStyle(
        settings.fontHeaderFamily || 'Times New Roman',
        toNumber(settings.fontHeaderSize, 14),
        settings.fontHeaderBold,
        settings.fontHeaderItalics,
        settings.fontHeaderUnderline,
        settings.fontHeaderStrikethrough
    );

    LAYOUT.FONT_SECTIONHEADER = buildFontStyle(
        settings.fontSectionFamily || 'Times New Roman',
        toNumber(settings.fontSectionSize, 11),
        settings.fontSectionBold,
        settings.fontSectionItalics,
        settings.fontSectionUnderline,
        settings.fontSectionStrikethrough
    );

    LAYOUT.FONT_FIELDLABEL = buildFontStyle(
        settings.fontFieldLabelFamily || 'Times New Roman',
        toNumber(settings.fontFieldLabelSize, 9),
        settings.fontFieldLabelBold,
        settings.fontFieldLabelItalics,
        settings.fontFieldLabelUnderline,
        settings.fontFieldLabelStrikethrough
    );

    LAYOUT.FONT_FIELDOUTPUT = buildFontStyle(
        settings.fontFieldOutputFamily || 'Times New Roman',
        toNumber(settings.fontFieldOutputSize, 9),
        settings.fontFieldOutputBold,
        settings.fontFieldOutputItalics,
        settings.fontFieldOutputUnderline,
        settings.fontFieldOutputStrikethrough
    );
}
...
class DevExpressConverter
{
    ...
    {
        // Mini module can run without the full tool settings UI.
        if (document.getElementById('settings_pageWidth')) {
            applyLayoutSettingsFromUI();
        }
    }
...
create or alter procedure [${procedureName}_Printout]
  @FormDataGUID varchar(36) = null,
  @OwnerObjectGUID varchar(36) = null
as
  /*
...
GO`);
        ...
        // ? Show output container when present
        const outputSql = document.getElementById('outputSql');
        if (outputSql) {
            outputSql.style.display = 'block';
        }
...
const UIHandlers = {
    ...
    `;
        ...
        timestamp.textContent = `Error occurred on ${new Date().toLocaleString()} (${getTimeZoneAbbr()})`;
        duration.innerHTML = `
